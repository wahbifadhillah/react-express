...
function EditPost() {
	...
	const fetchData = async () => {
		try {
			const res = await axios.get(`http://localhost:8000/article/${id}`);
			setData(res.data);
		} catch (err) {
			console.error(err);
		} finally {
	...
	useEffect(() => {
		if (!Object.keys(data).length) fetchData();
	});

	const formik = useFormik({
		initialValues: { title: "", content: "", category: "", status: "draft", ...data },
		...
		onSubmit: async (values) => {
			...
			try {
				await axios.patch(`http://localhost:8000/article/${id}`, { title, content, category, status });
			} catch (err) {
				console.error(err);
			} finally {
				navigate("/");
	...
	return (
		<Layout title="Edit Post" isLoading={isLoading}>
			<Box
				sx={{
					width: 500,
					maxWidth: "100%",
